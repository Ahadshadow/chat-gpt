...
const { isMobileMenu, isMenuOpen, isMenuOpening, isMenuClosing } =
...
const sortedConversations = computed(() =>
  conversationsV2.value.slice(1).sort((a, b) => b.uuid - a.uuid)
);

// const sortedConversations = JSON.parse(
//   localStorage.getItem("conversations/v2")
// );
...
const updateConversationTitleHandler = (event) => {
  event.stopPropagation();
  updateConversationTitle(
    conversationTitleToEditId.value,
    conversationTitleToEditValue.value
  );
...
<template>
  <div
    class="flex flex-col bg-purple-300/10 flex-1 max-w-xs shadow-lg absolute top-0 left-0 h-screen z-50 w-full backdrop-blur-lg transition-all duration-300 ease-in-out"
    :class="{
  ...
  >
    ...
    <Transition name="fade">
      ...
      >
        <button
          ...
          class="bg-purple-300/10 rounded-r text-white/50 hover:text-white/80 focus:outline-none z-10 transition duration-300 ease-in-out"
        >
    ...
    <div class="p-3 pb-0">
      ...
      <button
        ...
        class="flex flex-row items-center w-full bg-white/5 rounded shadow p-3 mb-3 transition duration-300 ease-in-out hover:bg-white/10"
    ...
    <div class="flex-1 flex flex-col gap-1 p-3 overflow-y-auto">
      <TransitionGroup name="slide-from-left">
        <div
          ...
          :class="{
            '!border-purple-300/20': conversation.id === currentConversationId,
          }"
        >
          <div
          ...
          >
            ...
            >
              <span>
                <GPTIcon
                  v-if="
                    (conversation.activePreset?.client ||
                      conversation.activePresetName) === 'chatgpt'
                  "
                ...
                <GPTIcon
                  v-else-if="
                    (conversation.activePreset?.client ||
                      conversation.activePresetName) === 'chatgpt-browser'
                  "
              ...
              <span v-else class="flex-1 truncate flex items-center gap-1">
            ...
            <span class="text-xs text-white/40">
              {{ new Date(conversation.uuid).toLocaleString() }}
            </span>
          </div>
          <button
    ...
    <!-- Sidebar footer -->
    <div
      class="flex flex-col items-start gap-2 bg-black/10 p-6 min-h-[100px] text-sm"
    >
      <button
      ...
      >
        <Icon
          :name="isConfirmingClear ? 'bx:bx-check' : 'bx:bx-trash'"
